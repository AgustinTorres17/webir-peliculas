...
export const MovieData = ({ movie, platforms }) => {
  return (
    <div className="flex flex-col lg:flex-row gap-10 p-5 w-full">
      ...
      <div className="flex flex-col items-start lg:items-start gap-2">
        <h2 className="text-xl font-medium text-accent">
          Puedes verla en estas plataformas
        </h2>
        <div className="grid grid-cols-3 gap-2 justify-items-start lg:flex lg:gap-x-10">
          {platforms &&
            platforms.map((platform) => (
              <div
                key={platform.name}
                className="h-20 w-20 lg:h-24 lg:w-24 rounded-lg overflow-hidden shadow-xl flex justify-center hover:scale-110 hover:cursor-pointer hover:shadow-xl transition-all duration-400"
              >
                <img
                  src={`${platform.logo}`}
